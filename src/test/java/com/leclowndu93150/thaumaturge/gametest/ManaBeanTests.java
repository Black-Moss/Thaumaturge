...
public final class ManaBeanTests {
    ...
    private static BlockEntityManaPod placePod(GameTestHelper helper, BlockPos pos, int age,
                                               ResourceKey<IAspect> aspect) {
        helper.setBlock(pos.above(), Blocks.OAK_LOG);
        helper.getLevel().setBlock(helper.absolutePos(pos),
                TCBlocks.MANA_POD.get().defaultBlockState().setValue(BlockManaPod.AGE, age), 3);
    ...
    public static void register(TCTestRegistrar r) {
        r.add("manabean/combination_lookup", 20, helper -> {
            ...
            Holder<IAspect> combo = AspectCombinations.result(registries,
                    aspects.getOrThrow(TCAspects.AER), aspects.getOrThrow(TCAspects.ORDO));
        ...
        r.add("manabean/pod_growth_assigns_aspect", 60, helper -> {
            ...
            int age = helper.getLevel().getBlockState(helper.absolutePos(new BlockPos(2, 2, 2)))
