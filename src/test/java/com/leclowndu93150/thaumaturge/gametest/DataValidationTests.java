...
public final class DataValidationTests {
        ...
        r.add("data/aspect_compositions_resolve", 20, helper -> {
            ...
            registry.listElements().forEach(aspect -> {
                if (!aspect.value().isPrimal() && !terminatesAtPrimals(aspect, 0, new HashSet<>())) {
                    helper.fail("Aspect " + aspect.key().identifier()
                            + " does not decompose to primals within depth " + MAX_COMPONENT_DEPTH);
                }
            });
            helper.succeed();
        });
    ...
    private static void requireRecipes(net.minecraft.gametest.framework.GameTestHelper helper,
                                       RecipeManager recipes, RecipeType<?> type, String label) {
        boolean any = recipes.getRecipes().stream()
                .anyMatch(holder -> holder.value().getType() == type);
        if (!any) {
