    ...
    public static void register(TCTestRegistrar r) {
        r.add("relay/links_and_drains", 300, helper -> {
            ...
            helper.runAfterDelay(ACCRUE_WAIT_TICKS, () -> {
                ...
                int drained = VisRelayHelper.drainCentivis(helper.getLevel(),
                        helper.absolutePos(CONSUMER_POS), TCAspects.IGNIS, 100, false);
                if (drained <= 0) {
                    helper.fail("Nothing drained through the relay after "
                            + ACCRUE_WAIT_TICKS + " ticks of accrual");
                    return;
                }
        ...
        r.add("relay/chains_through_parent_relay", 60, helper -> {
            ...
            if (second.depth() != 2 || !helper.absolutePos(RELAY_POS).equals(second.parentPos())) {
                helper.fail("Second relay did not chain through the first (depth "
                        + second.depth() + ", parent " + second.parentPos() + ")");
                return;
            }
            helper.succeed();
        });
