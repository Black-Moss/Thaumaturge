...
public final class InhabitedZombieRenderer extends AbstractZombieRenderer<EntityInhabitedZombie, ZombieRenderState, ZombieModel<ZombieRenderState>> {
    ...
    private static ItemStack zombiePlate(ItemStack stack) {
        ...
        if (equippable == null
                || !equippable.assetId().map(TCMaterials.ASSET_CULTIST_PLATE::equals).orElse(false)) {
            return stack;
        }
        ItemStack copy = stack.copy();
        copy.set(DataComponents.EQUIPPABLE, new Equippable(
                equippable.slot(), equippable.equipSound(), Optional.of(TCMaterials.ASSET_ZOMBIE_PLATE),
