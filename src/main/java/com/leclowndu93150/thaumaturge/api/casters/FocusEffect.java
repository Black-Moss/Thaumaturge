...
/**
 * An element that applies the spell's payload to a resolved hit, such as damage, a potion effect, or block interaction.
 *
 * @since 1.0.0
 */
public non-sealed interface FocusEffect extends FocusElement {
    ...
    /**
     * Applies this effect to one target. Called once per supplied target; the spell's current power is
     * {@link CastContext#power()}.
     *
    ...
    /**
     * Called once on the server when a spell containing this effect is cast, before any node executes.
     *
    ...
    /**
     * Spawns this effect's travel or impact particles without carrier drift. Called on the client only.
     *
     ...
     *
     * <p>
     * The drift is a carrier velocity the particle applies every tick of its life, unaffected by its own friction or
     * gravity. Cast bursts pass the caster's velocity here so the flash keeps pace with a moving caster.
     *
     * @param level the client level
     * @param pos the particle position
     * @param motion the particle motion
