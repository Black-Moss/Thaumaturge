 ...
 *
 * <p>
 * A medium either resolves its output immediately, or defers by launching an intermediary such as a projectile entity:
 * it spawns the carrier with {@link CastContext#continuation()} and returns null, which halts the current branch. The
...
public non-sealed interface FocusMedium extends FocusElement {
     ...
     * @return the streams for the next node, or null to halt the branch because target resolution was deferred to an
     *         intermediary or the medium could not act
     */
    @Nullable
