...
/**
 * The computed cost of crafting an {@link IArcaneRecipe} at a workbench for a given player, and where that cost is paid
 * from.
 *
 * <p>
 * An arcane craft is paid in three ways, resolved in order: primal vis drawn from a wand in the workbench's wand slot,
 * essentia crystals consumed from the crystal slots, and aura vis drained from the block entity. This record reports
 * the outcome of that resolution so addons can display the cost, decide affordability, or drive automation without
 * recomputing the rules.
 *
 * <p>
 * Instances are produced by {@link #of(IArcaneRecipe, IArcaneWorkbench, Player)} and are delivered mutably-replaceable
 * through {@link ArcaneCraftCostEvent}. All fields are immutable views.
 *
...
public record ArcaneCraftCost(boolean paidFromWand, Map<ResourceKey<IAspect>, Integer> wandCentivis, AspectList crystalsNeeded, int auraVis, boolean affordable) {
    ...
    /**
     * Implementation hook supplied by Thaumaturge that computes a cost from a recipe, workbench, and player. Addons must
