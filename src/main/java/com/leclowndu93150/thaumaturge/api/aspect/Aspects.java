 ...
 *
 * <p>
 * Addons may safely hold {@code ResourceKey<IAspect>} constants in static initializers because keys do not touch the
 * registry. A {@link Holder} is required before an aspect can be placed into an {@link AspectList} or compared, and
 * resolving one needs a registry view. These helpers provide that view from either a {@link HolderLookup.Provider} or a
 * {@link Level}.
 *
 * <p>
 * Every method returns {@code null} when the registry is unavailable or the key is unregistered, so callers holding
 * keys for aspects that may not exist can resolve defensively.
 *
 * @since 1.0.0
 */
