...
public final class ArcaneBoreScreen extends AbstractTCContainerScreen<MenuArcaneBore> {
    ...
    protected void extractBackgroundOverlay(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.text(font, Component.translatable("gui.thaumaturge.bore.width", 1 + bore.getDigRadius() * 2),
                0, 0, COLOR_WHITE, true);
        graphics.text(font, Component.translatable("gui.thaumaturge.bore.depth", bore.getDigDepth()),
                STATS_COLUMN_2, 0, COLOR_WHITE, true);
        graphics.text(font, Component.translatable("gui.thaumaturge.bore.speed",
        ...
        if (silk || refining > 0 || fortune > 0) {
            graphics.text(font, Component.translatable("gui.thaumaturge.bore.properties"),
                    0, PROPS_HEADER_Y, COLOR_WHITE, true);
        }
        int lineY = PROPS_FIRST_Y;
        if (refining > 0) {
            graphics.text(font, Component.translatable("gui.thaumaturge.bore.refining", refining),
                    PROPS_INDENT, lineY, COLOR_REFINING, true);
            lineY += PROPS_LINE_STEP;
        }
        if (fortune > 0) {
            graphics.text(font, Component.translatable("gui.thaumaturge.bore.fortune", fortune),
                    PROPS_INDENT, lineY, COLOR_FORTUNE, true);
            lineY += PROPS_LINE_STEP;
        }
        if (silk) {
            graphics.text(font, Component.translatable("gui.thaumaturge.bore.silktouch"),
