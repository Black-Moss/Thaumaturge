...
public final class ManaPodRenderer implements BlockEntityRenderer<BlockEntityManaPod, ManaPodRenderState> {
    ...
    @Override
    public void extractRenderState(BlockEntityManaPod pod, ManaPodRenderState state, float partialTicks,
    ...
    public void submit(ManaPodRenderState state, PoseStack poseStack, SubmitNodeCollector collector, CameraRenderState camera) {
        ...
        float b = HERBA_B;
        if (state.hasAspect) {
            float progress = Mth.clamp((state.age - SHELL_MIN_AGE)
        ...
        if (state.age > CORE_MIN_AGE - 1) {
            ...
            collector.submitModelPart(model.core, poseStack, RenderTypes.entityCutout(CORE_TEXTURE),
                    FULLBRIGHT, OverlayTexture.NO_OVERLAY, null, -1, null);
            poseStack.popPose();
        }
        int shellColor = ARGB.colorFromFloat(SHELL_ALPHA, r, g, b);
        collector.submitModelPart(model.shell, poseStack, RenderTypes.entityTranslucent(SHELL_TEXTURE),
                state.lightCoords, OverlayTexture.NO_OVERLAY, null, shellColor, null);
        poseStack.popPose();
    }
}
