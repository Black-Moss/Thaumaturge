...
public final class MirrorRenderer implements BlockEntityRenderer<BlockEntityMirrorBase, MirrorRenderState> {
    ...
    public void extractRenderState(
        ...
        boolean essentia =
                mirror.getBlockState().getBlock() instanceof BlockMirror mirrorBlock && mirrorBlock.isEssentia();
        BlockStateModel frameModel = Minecraft.getInstance()
                .getModelManager()
    ...
    @Override
    public void submit(
            MirrorRenderState state, PoseStack poseStack, SubmitNodeCollector collector, CameraRenderState camera) {
        poseStack.pushPose();
        ...
        if (!state.frameParts.isEmpty()) {
            ...
            collector.submitMultiLayerBlockModel(
                    poseStack, state.frameParts, true, new int[0], state.lightCoords, OverlayTexture.NO_OVERLAY, 0);
            poseStack.popPose();
        }
        if (state.linked && state.inRange) {
            collector.submitCustomGeometry(
                    poseStack,
                    EldritchPortalSurface.SURFACE,
                    (pose, buffer) -> portalWindow(pose, buffer, state.blockPos));
        }
        RenderType paneType =
                RenderTypes.entityTranslucent(state.linked && state.inRange ? PANE_TRANS_TEXTURE : PANE_TEXTURE);
